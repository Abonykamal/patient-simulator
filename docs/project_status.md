...
- [~] Second scenario file — **dropped** (low value): the app always generates dynamically and serves no authored scenario JSON; `chest_pain.json` stays a test fixture. Would only matter with a deterministic offline "demo mode", which was decided against.
- [x] RAG tested with multiple scenario types — `scripts/smoke_rag_specialties.py` (hand-run, live): all 5 corpus specialties (chest_pain, dyspnea, abdominal_pain, headache, leg_swelling) verified end to end — category metadata filter is a hard guarantee, each generates schema-valid on attempt 0, builds into a graph, yields a rubric, and the patients are distinct with clean door-stem intros (run 19-June-2026)
...
- [~] Demo recording — **dropped** (not being pursued; the project isn't being screen-recorded)

---

## What's Next
Phase 7 done; **Phase 8 (Polish) essentially complete** — the system is functionally complete end to end (create → converse → evaluate), has had a round of live-run quality fixes (ADR-032 Refinements), is edge-case hardened (ADR-033), ships a portfolio README, and now has the RAG pipeline verified live across all five specialties (`scripts/smoke_rag_specialties.py`). Two checklist items were **dropped, not done**: the second authored scenario (low-value — the app always generates dynamically and serves no authored scenario JSON; it would only matter with a deterministic offline "demo mode", which we decided against) and the demo recording (the project isn't being screen-recorded). Known, deliberately-deferred items: generation variety is **tabled** (only matters for a real product, not the portfolio MVP — structured input parameterisation is the recorded future lever; the cross-specialty run incidentally showed low *name* variety, e.g. a recurring surname, consistent with this); no list/resume of past sessions. Completed sessions now **do** block further turns (ADR-033). The "generally healthy"-style answer-phrased labels are handled by the judge's `not_applicable`/topic interpretation rather than a generator field.
...
Note: the live network path is exercised only by the four hand-run smoke scripts — `scripts/smoke_generator.py` (corpus → embed → retrieve → `scenario_generator` LLM → schema-validate → graph), `scripts/smoke_rag_specialties.py` (the same chain across all 5 specialties + category-filter and distinctness checks), `scripts/smoke_conversation.py` (full loop: `start_session` → `run_turn` × N with live agents), and `scripts/smoke_evaluation.py` (the live judge over a seeded transcript). Automated tests still make **no** real provider calls — every LLM collaborator is injected/faked; the smoke scripts are the deliberate, hand-run exceptions.
