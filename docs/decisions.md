...
**Decision:**
**Rubric derived from nodes** — each clinically-weighted node is a topic to ask about, weighted by the `importance` carried since ADR-017 (`critical=3, relevant=2, minor=1`). `build_rubric` keeps only `critical`/`relevant` nodes; `minor` incidental colour (occupation, living situation) stays in the graph for realism but is not graded. The **judge classifies** each rubric item `asked` / `not_asked` / `not_applicable` (+ a reasoning narrative); **code computes** the score (weighted coverage, dropping `not_applicable` from the denominator) and **formats** the report around the narrative. A dedicated **`evaluator.py`** coordinates. `POST /evaluate` is **idempotent** (returns the existing evaluation, no re-judge), **ends + judges + saves** in one action, and the judge **fails loud** (`503`, no fallback). The judge prompt is **process-based** (ADR-011): grade asking, not the patient's answers; "asked" needs a student utterance explicitly seeking the info (incl. clinical paraphrase); evidence is never inferred from the patient's reply.
...
**Refinements (live-run-driven, 2026-06-18).** Driving the app surfaced rubric-quality issues the fake-injected tests could not:
- *`minor` trivia graded* ("hairdresser", "lives alone") → `build_rubric` drops `minor` (kept the nodes for realism). Category filtering was rejected — "smoker" and "hairdresser" are both `social`, so category can't separate signal from noise; `importance` can.
- *Non-askable nodes graded* (findings/observations like "stable vital signs", "downplaying symptoms") and *answer-phrased labels* ("generally healthy") → handled by the judge's new **`not_applicable`** verdict, not by a node field. A `probe` field on the generator was prototyped then rejected: it adds generator LLM-surface and biasing examples, whereas the judge already reads the labels and is the natural place to decide askability. Trade-off accepted: askability is decided per grading run (the score was already LLM-variable).
- *Scenario intro leaked elicitable history* (the door-stem stated the timeline + key risk factor) → the generator's `scenario_intro` instruction now demands a name/age/presenting-complaint-only stem, phrased schematically (`"<name>, <age>, presents with <complaint>"`) to avoid anchoring generation toward one case.
- *Low generation variety per category* (only 2–4 corpus cases per category → near-identical grounding) → **tabled**: not needed for the project's portfolio/MVP goal; it is a real-product / scale-up concern. The structured-parameterisation lever (control variation at the input) is recorded as the future approach if the project grows.
